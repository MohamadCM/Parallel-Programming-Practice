...
bool checkCorrectness(float* mat1, float* mat2, float* res, int size);
/**
...
int matrixMultiply(int argc, char** argv, int n)
{
	...
	//if (checkCorrectness(h_A, h_B, h_C, n))
	//	printf("\nParallel result is correct!\n");
	//else
	//	printf("\nParallel result is wrong!\n");
...
bool checkCorrectness(float* mat1, float* mat2, float* res, int size) {
	float* sum = (float*)malloc(size * size * sizeof(float));
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			sum[i * size + j] = 0;
			for (int k = 0; k < size; k++)
				sum[i * size + j] += mat1[i * size + k] * mat2[k * size + j];
		}
	}
	for (size_t i = 0; i < size; i++) {
		for (size_t j = 0; j < size; j++) {
			if (sum[i * size + j] != res[i * size + j])
				return false;
		}
	}
	return true;
